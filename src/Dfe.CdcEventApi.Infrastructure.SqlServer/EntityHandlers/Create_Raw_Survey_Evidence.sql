...
SELECT
    ...
    Tbl.Col.value('ref_SurveyEvidenceItemTypeReference[1]', 'nvarchar(max)') AS [ref_SurveyEvidenceItemTypeReference],
...
LEFT JOIN (SELECT [bk_id] FROM [raw].[Survey_Evidence] WHERE [Load_DateTime] = @RunIdentifier) AS [existing]
